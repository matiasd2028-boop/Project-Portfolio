...
const projectsData = {
  "rc-car": {
    ...
    fallbackGradient: "linear-gradient(135deg, #1e1b4b 0%, #311042 100%)",
    links: []
  },
  "weather-balloon": {
  ...
  "axiom-chess": {
    ...
    status: "wip", // Marked as In Progress for active updates
    ...
    fallbackGradient: "linear-gradient(135deg, #022c22 0%, #0f172a 100%)",
    links: [
  ...
  "openclaw-ai": {
    ...
    status: "wip", // Marked as In Progress to highlight active system expansion
    ...
    fallbackGradient: "linear-gradient(135deg, #172554 0%, #1e1b4b 100%)",
    links: []
  },
  "custom-nas": {
  ...
  "solar-tracker": {
    ...
    fallbackGradient: "linear-gradient(135deg, #451a03 0%, #172554 100%)",
    links: []
  },
  "fraud-detector": {
    ...
    image: "", // Use fallback gradient (no generated image due to quota limit)
    fallbackGradient: "linear-gradient(135deg, #581c87 0%, #0f172a 100%)",
    links: [
  ...
  "world-cup": {
    ...
    image: "", // Use fallback gradient (no generated image due to quota limit)
    fallbackGradient: "linear-gradient(135deg, #1e1b4b 0%, #164e63 100%)",
    links: [
      { text: "View GitHub Repository", url: "https://github.com/matiasd2028-boop/World-Cup-Predictor", type: "github" }
    ]
  },
  "autonomous-drone": {
    title: "Autonomous Drone Navigation (WIP)",
    status: "wip", // Custom Project added to showcase academic drive & technical expansion
    result: "Developing an autonomous quadcopter pathfinding system utilizing ROS and light weight computer vision models.",
    skills: ["C++ Programming", "ROS (Robot Operating System)", "Sensor Fusion (IMU/GPS)", "Edge Computing"],
    challenges: "Optimizing neural network weights for low-power edge deployment and filtering telemetry sensor noise in real-time flight.",
    image: "",
    fallbackGradient: "linear-gradient(135deg, #4c1d95 0%, #0369a1 100%)",
    links: []
  }
};
...
function setupTabFilters() {
  ...
  tabs.forEach(tab => {
    tab.addEventListener("click", () => {
      ...
      tabs.forEach(t => {
        t.classList.remove("text-cyan-400", "border-cyan-400", "bg-cyan-950/20");
        t.classList.add("text-gray-400", "border-transparent");
      });
      tab.classList.remove("text-gray-400", "border-transparent");
      tab.classList.add("text-cyan-400", "border-cyan-400", "bg-cyan-950/20");
  ...
  cards.forEach(card => {
    card.addEventListener("click", () => {
      ...
      if (data.status === "completed") {
        ...
        modalStatus.className = "px-2 py-0.5 text-xs font-semibold rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 badge-completed";
      } else {
        modalStatus.innerText = "Work In Progress";
        modalStatus.className = "px-2 py-0.5 text-xs font-semibold rounded-full bg-amber-500/10 text-amber-400 border border-amber-500/20 badge-wip";
      }
      ...
      if (data.image) {
        ...
        placeholderDiv.innerHTML = `
          <div class="p-4 rounded-full bg-white/5 border border-white/10 backdrop-blur-md mb-3">
            <svg class="w-12 h-12 text-cyan-400" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" d="M17.25 6.75L22.5 12l-5.25 5.25m-10.5 0L1.5 12l5.25-5.25m7.5-3l-4.5 16.5" />
            </svg>
          </div>
          <span class="text-white text-lg font-bold">${data.title}</span>
          <span class="text-xs text-cyan-200 mt-1 uppercase tracking-widest font-semibold">Visual Abstract Placeholder</span>
        `;
      ...
      data.skills.forEach(skill => {
        ...
        chip.className = "px-3 py-1 text-sm rounded-lg bg-slate-900 border border-slate-700/60 text-cyan-300 font-medium shadow-sm";
      ...
      if (data.links && data.links.length > 0) {
        data.links.forEach(link => {
          ...
          if (link.type === "github") {
            btn.className += "bg-slate-800 hover:bg-slate-700 text-white border border-slate-700 shadow-slate-900/50 hover:shadow-cyan-500/10";
            btn.innerHTML = `
            ...
            btn.className += "bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 text-white shadow-violet-900/40 hover:shadow-violet-500/20";
            btn.innerHTML = `
              <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z"/></svg>
              <span>View Report</span>
            `;
          } else if (link.type === "grade") {
            // Unclickable or informational Grade Report badge
            btn.href = "javascript:void(0)";
            btn.className = "bg-emerald-950/40 hover:bg-emerald-950/60 text-emerald-400 border border-emerald-700/50 cursor-default";
            btn.innerHTML = `
